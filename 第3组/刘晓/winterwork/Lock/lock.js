(function(w){
  ...
  Lock.prototype = {
    ...
    createCanvas: function(){ // 创建 canvas
      var width, elRect;
      elRect = this.el.getBoundingClientRect();//获得页面相对位置.getBoundingClientRect
    ...
    ,  drawCircles: function(){ // 画所有圆
      this.ctx.clearRect(0, 0, this.width, this.width); // 为了防止重复画,.clearRect删除
      for(var i = 0; i < this.circles.length; i++){
        this.drawCircle(this.circles[i].x, this.circles[i].y,"i");
      }
    },
    ...
    drawCircle: function(x, y, color){ // 画圆
      ...
      this.ctx.beginPath();
      this.ctx.arc(x, y, this.r, 0, Math.PI * 2, true);//画圆.arc
      this.ctx.closePath();
      this.ctx.stroke();
    },
    getTouchPos: function(e){ // 获得触摸点的相对位置
      var rect = e.target.getBoundingClientRect();//获得页面相对位置.getBoundingClientRect
      var p = { // 相对坐标
    ...
    judgePos: function(p){ // 判断 触点 是否在 circle 內
      for(var i = 0; i < this.restCircles.length; i++){
        ...
        if(Math.abs(p.x - temp.x) < r && Math.abs(p.y - temp.y) < r){       //(p.x是触摸处，temp.x是未连接点
          this.touchCircles.push(temp);
          this.restCircles.splice(i, 1); //splice()方法向/从数组中添加/删除项目，然后返回被删除的项目
          this.touchFlag = true;
          this.reDraw = true;
          break;
        }
    ...
    drawLine2TouchPos: function(p){//动态监听绘点
      var len = this.touchCircles.length;
      if(len >= 1){
...
//下面的没看
    showInfo: function(message, timer){ // 专门用来显示 info
      clearTimeout(this.showInfo.timer);//clearTimeout() 方法可取消由 setTimeout() 方法设置的 timeout。
    ...
    checkPass: function(){ // 判断当前 model 和检查密码
      ...
      if(model == 2){// 设置密码
        ...
        if(!flag){
          ...
          setTimeout(window.open("http://www.hao123.com"),2000)//跳转页面
        }
      }
      if(succ){         //添加color颜色变化
    ...
    createListener: function(){ // 创建监听事件
      ...
      this.dom.setPass.addEventListener('click', function(e){     //手动重置密码
        self.lsPass.model = 2;
        self.updateMessage();
